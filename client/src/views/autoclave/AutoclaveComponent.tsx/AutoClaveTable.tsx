...
const navigate = useNavigate()
  ...
  const totalPages = Math.ceil((autoclavedata?.length || 0) / pageSize);
  const currentItems = autoclavedata?.slice((currentPage - 1) * pageSize, currentPage * pageSize) || [];
...
navigate(`/autoclave-view/${autoclave?.autoclave_entries[0]?.id}`)
  ...
  return (
    <div>
              ...
              currentItems.map((item, index) => (
                <tr key={item.id} className="bg-white dark:bg-gray-900">
                  ...
                  <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    ...
                    {item?.autoclave_entries[0]?.operator_name.charAt(0).toUpperCase() + item?.autoclave_entries[0]?.operator_name.slice(1)||"-"}

                  </td>
                
                  <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    {item?.autoclave_entries[0]?.
                      on_time
                      ? new Date(`1970-01-01T${item.autoclave_entries[0]?.
      ...
      />
      <AddAutoClaveModal setShowmodal={setAddmodal} show={addmodal} batchingData={selectedrow} />
