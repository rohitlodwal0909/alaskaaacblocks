...
import { addBoiler, GetBoiler } from "src/features/Boiler/BoilerSlice";
import { LocalizationProvider, TimePicker } from "@mui/x-date-pickers";
import { AdapterDayjs } from "@mui/x-date-pickers/AdapterDayjs";
import dayjs from "dayjs";

const AddBoilerModal = ({ show, setShowmodal, logindata }) => {
 ...
 const validateForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await dispatch(addBoiler(formData)).unwrap();
      toast.success(result.message || "Boiler entry created successfully");
      dispatch(GetBoiler());
      setFormData(initialData);
      setShowmodal(false);
    } catch (err) {
      toast.error("Failed to create Boiler entry");
    }
  };

  return (
    <Modal show={show} onClose={() => setShowmodal(false)} size="5xl">
      <ModalHeader>Add Boiler Entry</ModalHeader>
