...
import AddROwaterModal from "./AddROwaterModal";
import EditROwaterModal from "./EditROwaterModal";
import { deleteROwater, GetROwater } from "src/features/RO-water/ROwaterSlice";


const ROwaterTable = () => {
  const dispatch = useDispatch<AppDispatch>();
  const { rowaterdata, loading } = useSelector((state: any) => state.rowater);
...
// const navigate = useNavigate()
  ...
  //  navigate(`/autoclave-view/${autoclave?.ROwater_entries[0]?.id}`)
  // }
  const handleDelete = async (userToDelete) => {
    if (!userToDelete) return;
    try {
      console.log
      await dispatch(deleteROwater(userToDelete?.ROwater_entries[0]?.id)).unwrap();
      dispatch(GetROwater());
        toast.success(" RO water  was successfully deleted. ");
    } catch (error: any) {
  ...
  const filteredItems = (rowaterdata || []).filter((item: any) => {
    const searchText = searchTerm.toLowerCase();
    const operator = item?.ROwater_entries?.[0]?.operator_name || "";
    const mouldNo = item?.mould_no || "";
    const okpeac = item?.ROwater_entries?.[0]?.no_of_ok_pcs || "";
    const brokenpeac = item?.ROwater_entries?.[0]?.no_of_broken_pcs || "";
    const size = item?.ROwater_entries?.[0]?.size || "";
    const date = item?.ROwater_entries?.[0]?.date || "";
    const remark = item?.ROwater_entries?.[0]?.remark || "";
  ...
  return (
    <div>
            ...
            {loading ? (
              ...
              currentItems.map((item, index) => (
                <tr key={item.id} className="bg-white dark:bg-gray-900">
                  ...
                  <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                            {item?.ROwater_entries[0]?.operator_name.charAt(0).toUpperCase() + item?.ROwater_entries[0]?.operator_name.slice(1)||"-"}
                  </td>
                
                
                     <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    {item?.ROwater_entries[0]?.no_of_ok_pcs || '-'}
                  </td>
                    <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    {item?.ROwater_entries[0]?.no_of_broken_pcs  || '-'}
                  </td>
                    <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    {item?.ROwater_entries[0]?.size || '-'}
                  </td>
                    <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    {item?.ROwater_entries[0]?.date|| '-'}
                  </td>
                    <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    {item?.ROwater_entries[0]?.remark || '-'}
                  </td>
                  <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    <div className="flex justify-start gap-2">
                      {
                        !item?.ROwater_entries[0]?.operator_name ?
      ...
      <ComonDeletemodal
        ...
        title="Are you sure you want to Delete this ROwater ?"
      />
      <AddROwaterModal setShowmodal={setAddmodal} show={addmodal} ROwaterdata={selectedrow} logindata={logindata}/>
      <EditROwaterModal show={editmodal} setShowmodal={setEditmodal} autoclave={selectedrow} />

    </div>
  );
};

export default ROwaterTable;
