...
import { updateROwater, GetROwater } from 'src/features/RO-water/ROwaterSlice';
// import { Icon } from '@iconify/react';

const EditROwaterModal = ({ show, setShowmodal, autoclave }) => {
  ...
  useEffect(() => {
    const segData = autoclave?.ROwater_entries?.[0];
    if (segData) {
      setFormData({
        id: segData?.id || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await dispatch(updateROwater(formData)).unwrap();
      toast.success(result.message || 'ROwater entry updated successfully');
      dispatch(GetROwater());
      setShowmodal(false);
    } catch (err) {
      toast.error('Failed to update ROwater entry');
    }
  };

  return (
    <Modal show={show} onClose={() => setShowmodal(false)} size="4xl">
      <ModalHeader>Edit ROwater Entry</ModalHeader>
...
export default EditROwaterModal;
