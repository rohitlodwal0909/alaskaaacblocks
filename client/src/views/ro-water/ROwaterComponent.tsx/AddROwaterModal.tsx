...
import { addROwater, GetROwater } from "src/features/RO-water/ROwaterSlice"; // Adjust path if needed

const AddROwaterModal = ({ show, setShowmodal, ROwaterdata ,logindata}) => {
  const dispatch = useDispatch<AppDispatch>();

  const [formData, setFormData] = useState({
     user_id:logindata?.admin?.id,
    mould_no: ROwaterdata?.mould_no || "",
    operator_name: "",
    size: "",
    no_of_broken_pcs: "",
    no_of_ok_pcs: "",
  ...
  useEffect(() => {
    setFormData((prev) => ({ ...prev, mould_no: ROwaterdata?.mould_no || "" }));
  }, [ROwaterdata]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await dispatch(addROwater(formData)).unwrap();
      toast.success(result.message || "ROwater entry created successfully");
      dispatch(GetROwater());
      ...
      toast.error("Failed to create ROwater entry");
    }
  };

  return (
    <Modal show={show} onClose={() => setShowmodal(false)} size="4xl">
      <ModalHeader>Create New ROwater Entry</ModalHeader>
...
export default AddROwaterModal;
