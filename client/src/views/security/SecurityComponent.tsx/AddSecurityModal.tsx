...
import { addSecurity, GetSecurity } from "src/features/Security/SecuritySlice"; // Adjust path if needed

const AddSecurityModal = ({ show, setShowmodal, Securitydata ,logindata}) => {
  const dispatch = useDispatch<AppDispatch>();

  const [formData, setFormData] = useState({
     user_id:logindata?.admin?.id,
    mould_no: Securitydata?.mould_no || "",
    operator_name: "",
    size: "",
    no_of_broken_pcs: "",
    no_of_ok_pcs: "",
  ...
  useEffect(() => {
    setFormData((prev) => ({ ...prev, mould_no: Securitydata?.mould_no || "" }));
  }, [Securitydata]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(result.message || "Security entry created successfully");
      ...
      toast.error("Failed to create Security entry");
    }
  };

  return (
    <Modal show={show} onClose={() => setShowmodal(false)} size="4xl">
      <ModalHeader>Create New Security Entry</ModalHeader>
