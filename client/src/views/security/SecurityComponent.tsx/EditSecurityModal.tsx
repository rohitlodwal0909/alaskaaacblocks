...
import { updateSecurity, GetSecurity } from 'src/features/Security/SecuritySlice';
// import { Icon } from '@iconify/react';

const EditSecurityModal = ({ show, setShowmodal, autoclave }) => {
  ...
  useEffect(() => {
    const segData = autoclave?.Security_entries?.[0];
    if (segData) {
      setFormData({
        id: segData?.id || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(result.message || 'Security entry updated successfully');
      dispatch(GetSecurity());
      setShowmodal(false);
    } catch (err) {
      toast.error('Failed to update Security entry');
    }
  };
