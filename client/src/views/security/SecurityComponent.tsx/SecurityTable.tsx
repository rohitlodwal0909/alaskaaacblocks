...
import { deleteSecurity, GetSecurity } from "src/features/Security/SecuritySlice";
...
// const navigate = useNavigate()
  ...
  //  navigate(`/autoclave-view/${autoclave?.Security_entries[0]?.id}`)
  // }
  const handleDelete = async (userToDelete) => {
    if (!userToDelete) return;
    try {
      console.log
      await dispatch(deleteSecurity(userToDelete?.Security_entries[0]?.id)).unwrap();
      dispatch(GetSecurity());
        toast.success("The Security  was successfully deleted. ");
    } catch (error: any) {
  ...
  const filteredItems = (securitydata || []).filter((item: any) => {
    ...
    const operator = item?.Security_entries?.[0]?.operator_name || "";
    const mouldNo = item?.mould_no || "";
    const okpeac = item?.Security_entries?.[0]?.no_of_ok_pcs || "";
    const brokenpeac = item?.Security_entries?.[0]?.no_of_broken_pcs || "";
    const size = item?.Security_entries?.[0]?.size || "";
    const date = item?.Security_entries?.[0]?.date || "";
    const remark = item?.Security_entries?.[0]?.remark || "";
  ...
  return (
    <div>
            ...
            {loading ? (
              ...
              currentItems.map((item, index) => (
                <tr key={item.id} className="bg-white dark:bg-gray-900">
                  ...
                  <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                            {item?.Security_entries[0]?.operator_name.charAt(0).toUpperCase() + item?.Security_entries[0]?.operator_name.slice(1)||"-"}
                  </td>
                
                
                     <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    {item?.Security_entries[0]?.no_of_ok_pcs || '-'}
                  </td>
                    <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    {item?.Security_entries[0]?.no_of_broken_pcs  || '-'}
                  </td>
                    <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    {item?.Security_entries[0]?.size || '-'}
                  </td>
                    <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    {item?.Security_entries[0]?.date|| '-'}
                  </td>
                    <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    {item?.Security_entries[0]?.remark || '-'}
                  </td>
                  <td className="whitespace-nowrap py-3 px-4 text-gray-900 dark:text-gray-300">
                    <div className="flex justify-start gap-2">
                      {
                        !item?.Security_entries[0]?.operator_name ?
      ...
      <ComonDeletemodal
        ...
        title="Are you sure you want to Delete this Security ?"
      />
      <AddSecurityModal setShowmodal={setAddmodal} show={addmodal} Securitydata={selectedrow} logindata={logindata}/>
      <EditSecurityModal show={editmodal} setShowmodal={setEditmodal} autoclave={selectedrow} />
