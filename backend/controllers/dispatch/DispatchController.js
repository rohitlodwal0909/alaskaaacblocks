...
const {
  validateAndUpdateFinishGood
} = require("../../helper/validateAndUpdateFinishGood");
...
exports.createDispatch = async (req, res) => {
  try {
    ...
    try {
      ...
      return res
        .status(400)
        .json({ error: "Invalid quantity_size_list format" });
    }

    // 🔁 VALIDATE and UPDATE Segregation table
    const validationResult = await validateAndUpdateFinishGood(
      sizeList,
      quantityList
    );
...
exports.updateDispatch = async (req, res) => {
  try {
    ...
    const segregationResult = await validateAndUpdateFinishGood(
      sizeList,
      quantityList
    );
